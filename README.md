...
It is ok to leave the TCD in this mode, especially if it is mounted in a car or places with no WiFi networks available. In a typical home setup, however, you might want to connect the TCD to your local WiFi network. To do so, navigate to the Config Portal and click on "WiFi Configuration". The bare minimum is to select an WiFi network name (SSID) and a WiFi password.
...
Clicking this button (and selecting "yes" in the confirmation dialog) erases the currently saved WiFi network connection and reboots the device; it will restart in "access point" (AP) mode. See [here](#connecting-to-a-wifi-network).
