 ...
 * The menu
 * 
 * The menu is involked by hold down the ENTER button for 3 seconds.
 ...
 *    - quit the menu ("END")
 *  ENTER cycles through the list, holding ENTER for 3 seconds selects an item, ie a mode.
 ...
 *        hour mode, which is then converted to 12 hour mode with AM/PM accordingly.)
 ...
 *      - Hold ENTER for 3 seconds to select the value and exit menu ("SAVE" is displayed briefly)
 ...
 *        custom times are ignored. The value means "minutes" (hence "MINUTE")               
 *  If mode is "select brightness" (display shows "LVL")
 *      - Press ENTER to cycle through the possible levels (1-5)
 *      - Hold ENTER for 3 seconds to use current value and jump to next display
 *      - After third display, "SAVE" is displayed briefly and the menu is left automatically.
 *  If mode is "end"
 *      - Hold ENTER for 3 seconds to quit the menu
 */
...
void enter_menu() {
    ...
    isEnterKeyHeld = false;     // reset enter flags
    ...
    // DisplaySelect: Wait for ENTER to cycle through main menu (displays and modes), 
    // HOLD ENTER to select current menu "item"
    ...
    if(displayNum <= MODE_DEPT) {  
        ...
        uint16_t yearSet;   // hold new times that will be set, 
        uint16_t monthSet;  // all need to be same type since they're passed by ref
        uint16_t daySet;
        uint16_t minSet;
        uint16_t hourSet;
        ...
        if(!checkTimeOut()) {  
            ...
            if(displaySet->isRTC()) {  // this is the RTC display, set the RTC as quickly as
                                       // possible, as soon as set button pressed
                
                // Update RTC and do fake year if neccessary
...
{    
    ...
    //timeBuffer[0] = '\0';  // No, timeBuffer is pre-set with previous number
...
void myloop() 
{
    ...
    wifi_loop();
    audio_loop();     // Test, might block us for too long
}
